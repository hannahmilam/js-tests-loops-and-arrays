...
function elemsTimesLength(arr) {
    let x = []
    arr.forEach(element => {x.push(element * arr.length)})
    return x
}
...
function flightCost(destination, firstClass) {
    ...
    const price = flights.find(f => f.to === destination.toUpperCase())
    if (firstClass) {
        return price.prices.firstClass
    }
    return price.prices.standard
...
function bandMemberDetails(name) {
    for (let i = 0; i < theBand.members.length; i++) {
        let musician = theBand.members[i].name
        let instrument = theBand.members[i].instrument
        if(musician == name)
        {
            return `${musician} is in the band and plays the ${instrument}`
        }
    }
}
